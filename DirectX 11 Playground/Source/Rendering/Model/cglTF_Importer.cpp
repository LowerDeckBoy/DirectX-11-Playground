...
cglTF_Importer::cglTF_Importer(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, std::string_view Filepath)
{

}

bool cglTF_Importer::Import(ID3D11Device* pDevice, ID3D11DeviceContext* pContext, std::string_view Filepath)
{
	if (bLoaded)
	{	
		::OutputDebugStringA("[glTF] Model is already loaded!\n");
		return false;
	}

	assert(m_Device = pDevice);
	assert(m_Context = pContext);
...
void cglTF_Importer::ProcessNode(cgltf_data* pData, cgltf_node* pNode, glTF::Node* ParentNode, XMMATRIX ParentMatrix)
{
...
glTF::Mesh* cglTF_Importer::ProcessMesh(cgltf_data* pData, cgltf_mesh* pMesh, XMMATRIX Matrix)
{
	glTF::Mesh* newMesh{ new glTF::Mesh() };
	...
	{
		glTF::Primitive* newPrimitive{ new glTF::Primitive() };
...
glTF::Material* cglTF_Importer::ProcessMaterial(cgltf_data* pData, cgltf_primitive* pPrimitive)
{
	glTF::Material* newMaterial{ new glTF::Material() };
	...
	if (material->normal_texture.texture)
	{
		...
		newMaterial->NormalTexture = new Texture(m_Device, m_Context, texturePath);
	}
	...
	if (material->emissive_texture.texture)
	{
		...
		//auto msg{ files::glTF::GetTexturePath(m_ModelPath.data(), uri) + '\n' };
		//::OutputDebugStringA(msg.c_str());
		newMaterial->EmissiveFactor = *(XMFLOAT3*)material->emissive_factor;
		newMaterial->EmissiveTexture = new Texture(m_Device, m_Context, texturePath);
	}
	...
	if (pbrMaterial->base_color_texture.texture)
	{
		...
		newMaterial->BaseColorTexture = new Texture(m_Device, m_Context, texturePath);
	}
	...
	if (pbrMaterial->metallic_roughness_texture.texture)
	{
		...
		newMaterial->MetallicTexture = new Texture(m_Device, m_Context, texturePath);
	}
